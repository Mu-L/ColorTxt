...
- **`useAnchoredAppShellMenu.ts`**：锚定按钮的 Teleport 菜单（定位、外点关闭、Esc）；打开时登记 **`dismissibleOverlayStack`**，点击模式点阅读器先关弹层、当次单击不翻页（拖动仍可滚动）。
...
- **`shortcutService.ts`**：窗口级快捷键监听：按持久化绑定匹配并派发动作；**`EDIT_MODE_MONACO_DEFERRED_ACTIONS`**（编辑态 Monaco 内让出滚屏/查找等）；蒙版打开时 **`READER_SCROLL_SHORTCUT_ACTIONS`**（行滚/翻页/上下章）不拦截，其它动作仍生效；朗读中空格暂停/播放、左右换行，**`VOICE_READ_SCROLL_BLOCKED_ACTIONS`** 仍吞掉行滚/页滚/章节/查找。详见 [基础功能.md](./基础功能.md) → **「快捷键」**、**「Monaco 查找栏」**。
