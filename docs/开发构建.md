...
| `ColorSchemePanel.vue`                               | 配色弹窗容器：`ColorSchemeTabBar` + 上述三面板；可用 **`visibleTabs`** 限制标签（找书仅 `['reader']`，仍显示 TabBar）。<br>确定时按可见标签组装一次 **`ColorSchemeApplyPayload`**（`reader` / `highlight` / `lineation` 可选字段）并 **`emit('apply')`**；父组件 **`onApplyColorScheme`** 一次落盘（避免分事件导致删色未写入）。打开时从 props 同步草稿 |
