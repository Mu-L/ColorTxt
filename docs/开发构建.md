...
│       │   ├── readerPalettePresets.ts # 内置/用户阅读器配色方案
...
- **`webdav/`**：**`webDavClient.ts`**（PROPFIND/GET/PUT/MKCOL/DELETE）+ **`registerWebDavIpc.ts`**；密码经 **`SECRET_SLOT_WEBDAV_PASSWORD`**；见 [基础功能.md](./基础功能.md) → **「WebDAV 同步」**。
...
- **`appShell.css`**：根组件专用样式（由 `App.vue` 以 scoped 方式引入）：全屏顶/底/侧栏布局、正文区等。极简/全屏浮动层进出过渡类 **`chromeFloatHeader` / `chromeFloatFooter` / `chromeFloatSidebar`**（收起 `pointer-events: none`）；浮动顶/底/侧栏向阅读区一侧投影。
...
- **`useAppReaderChrome.ts`**：全屏 / 极简阅读时顶栏/底栏/侧栏悬停显隐与侧栏宽度拖拽；**`handleReaderChromeEscape`** 按层关蒙版、菜单、查找栏、浮动栏，全屏连按两次 Esc 退出（顶部胶囊 `fullscreenTip`：「按 2 次 ESC 退出全屏」/「再按 1 次 ESC 退出全屏」）。
...
- **`readerPalette.ts`**：阅读器表面色（背景、章节标题、Monaco txtr token）默认值与合并；**`ReaderSurfaceColorEnabled`** 控制引号内/括号内/标点/特殊标记/数字/字母是否使用独立色（默认全开，关闭时 **`resolveEffectiveReaderPalette`** 回退为正文色，色值仍保留）；色值覆盖存 **`readerPaletteOverridesLight` / `readerPaletteOverridesDark`**，开关覆盖存 **`readerPaletteColorEnabledOverrides`**（亮暗共用，仅持久化 `false`；读盘时若无新键则合并旧的 Light/Dark 两份）；`App.vue` 向 **`ReaderMain`** 传入合并后的**有效色**；`useAppShellThemeWatch` 写入 `html` 的 `--reader-bg`、`--reader-chapter-title`。
- **`readerPalettePresets.ts`**：内置/用户配色方案；**`findReaderPalettePresetByColors`** / **`ensureMatchedReaderPalettePreset`**（先用户后内置，匹配不到追加「自定义」）；**`toPersistedReaderPaletteState`** 供主窗口与找书共用落盘。不持久化选中方案 id。
...
- **`chapterStickyScroll.ts`**：注册 DocumentSymbolProvider 以驱动 Monaco `stickyScroll`（outlineModel）黏性章节大纲；禁用黏性条点击跳转；`refreshStickyChapterScrollWidget` 在大纲/装饰更新后关开 sticky 以套用章节标题样式（与 `--reader-chapter-title`、`1.2em` 一致）。是否启用由设置 **`stickyChapterTitleEnabled`**（**设置 → 阅读 → 启用粘性章节标题**，默认开）与 `ReaderMain` 的 **`streamLoading`** 共同决定 Monaco `stickyScroll.enabled`。翻页用 **`getStickyChapterScrollHeight`**（当前 DOM 条高）与 **`predictStickyChapterScrollHeight`**（按落点章节层数预测；candidate 与 DocumentSymbol 一致：至少跨三行，行数 capped 为 `maxLineCount` 与视口约 25% 的较小值）。详见 [基础功能.md](./基础功能.md) → **「黏性章节条」**。
- **`readerEditorOptions.ts`**：阅读器 `create` / `updateOptions` 的选项构建（换行、只读/编辑 chrome、小地图、行号、**`stickyScroll`**（受 **`stickyChapterTitleEnabled`** 控制）、**`disableMonospaceOptimizations`**（内嵌非等宽字体时纯 ASCII 行选区不错位）等）；垂直滚动条：**窗口只读 / 任意编辑** 为 `visible`（常显），**全屏只读** 为 `auto`（失焦淡出）。
- **`readerInlineDecorations.ts`**：章节标题行内装饰；Monaco 主题 chrome（小地图/滚动条/选区/当前行）；**`editor.background` 透明**以透出 **`--reader-bg`**；**`buildChapterMinimapSectionHeaderDecorations`**（编辑态小地图节标题）；合并 `readerPalette` 与 **`highlightColors`** 生成 Monarch token 规则；自定义高亮词开启时并入 `txtrHighlightMonarch` 生成的规则。
- **`readerMainMonaco.css`**（由 `ReaderMain` 引入）：小地图左侧阴影、滚动条轨道与滑块、概览尺层级（光标标记不被轨道遮挡）；全屏时小地图/滚动条/概览尺 `position: fixed` 贴视口右缘（见 **`appShell.css`**）。**圆角选区内角**：`.cslr.monaco-editor-background` 使用 **`--reader-bg`**（主题底色透明时否则列重叠的多行选区会连成一块）。
...
| `ColorSchemeTabBar.vue`                              | 配色弹窗内页签：**阅读器** / **高亮色** / **标注色**；右侧主题色图标按钮（与顶栏相同，`changeTheme` 立即改 shell 主题）+ **更多**（导出 / 导入配色） |
| `ColorSchemeReaderPanel.vue`                         | 「阅读器」页：表面色字段网格 + 实时预览（与 `ColorSchemePanel` 草稿联动）。<br>**`readerPane`**：`list` 方案卡片；`colors` 仅取色器（预览忽略开关、始终上色）；`switches` 同样展示全部色值，拾色器禁用，可选 token 行显示 **`SwitchToggle`**（关时预览回退正文色）。footer **「编辑配色」**（内置方案禁用）/ **「编辑开关」** / **「新增配色方案」**；编辑中为 **「退出编辑」**，用户方案改色时可 **重命名** / **删除** |
| `ColorSchemePresetList.vue`                          | 阅读器配色方案卡片网格（三列）：卡片背景为「背景色」、名称为「正文」色、下方其余 7 色块；选中边框 **`--accent`**；用户方案左上角彩色色盘图标（不可点，标识可改配色） |
...
| `ColorSchemePanel.vue`                               | 配色弹窗容器：`ColorSchemeTabBar`（含右侧主题色图标按钮，**`changeTheme`** 立即改 shell 主题；**更多** 导入/导出 JSON）+ 上述三面板；可用 **`visibleTabs`** 限制标签（找书仅 `['reader']`，仍显示 TabBar）。<br>阅读器页点选方案只替换亮/暗色值、**不改** token 开关（开关亮暗共用）。打开/导入后按亮/暗色值匹配方案（先用户、再内置；没有则草稿追加 **「自定义」**）。确定时按可见标签组装一次 **`ColorSchemeApplyPayload`** 并 **`emit('apply')`**；父组件 **`onApplyColorScheme`** / 找书 **`toPersistedReaderPaletteState`** 一次落盘。取消 / Esc 在草稿相对打开时有改动时确认。打开时从 props 同步草稿 |
