...
- **`appShell.css`**：根组件专用样式（由 `App.vue` 以 scoped 方式引入）：全屏顶/底/侧栏布局、正文区等。极简/全屏浮动层进出过渡类 **`chromeFloatHeader` / `chromeFloatFooter` / `chromeFloatSidebar`**（收起 `pointer-events: none`）。
