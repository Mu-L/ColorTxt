...
- **全局键**：覆盖层存活期间注册翻页/切章（**`@shared/stealthNavShortcuts`**，可由 **`setNavShortcuts`** 热更新）与退出键（跟随主窗「摸鱼模式」）；录制时经 `suspendStealthSessionShortcuts` 注销。退出或关覆盖层时注销热键、关闭设置窗并 `show` 源窗。Windows：指针进入任务栏条带（工作区外）或 **`WM_ACTIVATEAPP` / `browser-window-blur`** 后短延迟重申 **`setAlwaysOnTop(screen-saver, 1)` + `moveTop`**（菜单打开或热键隐藏时跳过），减轻点任务栏后被压住。
