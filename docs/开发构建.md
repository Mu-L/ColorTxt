...
| `ColorSchemeReaderPanel.vue`                         | 「阅读器」页：表面色字段网格 + 实时预览（与 `ColorSchemePanel` 草稿联动）。<br>除背景色/章节标题/正文外，引号内/括号内/标点/特殊标记/数字/字母各行标签前 **`SwitchToggle`**（默认开）；关时预览与 Monaco 使用正文色，**`HexColorPickerField`** 置灰禁用但保留已选色值；无开关项标签前留等宽占位以对齐。**「预设阅读器配色」** 时槽位换成 **`ColorSchemePresetList`** |
| `ColorSchemePresetList.vue`                          | 阅读器预设卡片网格（三列）：卡片背景为「背景色」、名称为「正文」色、下方其余 7 色块；选中边框 **`--accent`**；用户预设悬停显示改名 / 生成副本 / 删除；**「当前配色」** 悬停显示添加为预设 |
...
| `ColorSchemePanel.vue`                               | 配色弹窗容器：`ColorSchemeTabBar`（含右侧主题色图标按钮，**`changeTheme`** 立即改 shell 主题；**更多** 导入/导出 JSON）+ 上述三面板；可用 **`visibleTabs`** 限制标签（找书仅 `['reader']`，仍显示 TabBar）。<br>确定时按可见标签组装一次 **`ColorSchemeApplyPayload`**（`reader` / `highlight` / `lineation` 可选字段）并 **`emit('apply')`**；父组件 **`onApplyColorScheme`** 一次落盘（避免分事件导致删色未写入）。打开时从 props 同步草稿 |
