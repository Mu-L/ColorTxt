...
- **`shortcutService.ts`**：窗口级快捷键监听：按持久化绑定匹配并派发动作；**`EDIT_MODE_MONACO_DEFERRED_ACTIONS`**（编辑态 Monaco 内让出滚屏/查找等）；朗读中空格暂停/播放、左右换行，**`VOICE_READ_SCROLL_BLOCKED_ACTIONS`** 仍吞掉行滚/页滚/章节/查找。详见 [基础功能.md](./基础功能.md) → **「快捷键」**、**「Monaco 查找栏」**。
...
| `HeaderFontToolbar.vue`                              | 顶栏字体组：`FontPicker` + 字号 / 行间距加减；按钮 `title` / `aria-label` 为「当前值 → 点击后的值」（如「加大字号：24 → 25」「加大行间距：1.5 → 1.6」；已到上下限时只显示当前值）。宽屏由 **`AppHeader`** 直接渲染，窄屏经 **`MoreMenu` `#toolbar`** 插槽收纳；找书阅读器 **`FindBookReaderHeader`** 同样使用                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
