...
- **`readerColorSchemeExport.ts`**：配色 zip（`color-scheme.json` + `textures/`）；JSON 里已应用亮/暗色板带 **`textureId`**，用户方案为 `{ id, name, theme, palette }`，图项在 **`textures`** map（自定义含 `fileName` 与顶层 **`light` / `dark`**）。
