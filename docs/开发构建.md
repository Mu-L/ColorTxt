...
│       ├── assets/           # 字体与静态图标（含 eyedropper / image / options 等配色相关 SVG）
│       ├── styles/
...
- **定时滚动**：**`useAppTimedScroll`** 驱动顶栏 **定时滚动** 开关；按 **`timedScroll`** 设置间隔调用 **`ReaderMain.scrollByPageStep` / `scrollByLineStep`**（「一屏」与空格翻页同一套完整行重叠）；设置 **`timedScroll`** 持久化于 **`colorTxt.ui.settings`**（见 **「定时滚动」**）。
...
- **`icons.ts`**：各功能图标的 SVG 字符串汇总，供组件内联使用。配色相关含 **`eyedropper`** / **`image`** / **`options`**（`assets/*.svg`）。
...
- **`useAppChapterNavigation.ts`**：章节跳转、章节规则与最近文件、侧栏标签等联动；**`jumpToChapter`** 经 **`ReaderMain.scrollToLineNearTop`**，锚点字高带为 **`chapterJumpAnchorSlotFromTop(headingLevel)`**（与黏性章节条层数对齐，见 **「黏性章节条」**）；只读展示正文变更后重算章节——`.txt` 用 **`buildChaptersFromReaderDisplayText` / `buildChaptersFromReaderDisplayLines`**；`.md` 只读用 **`buildChaptersFromMarkdownPhysicalLines`**（物理行识别 ATX；展示行已去 `#` 时 **`title` 取展示行**，仍带 `#` 的编辑态展示行再经 **`detectMarkdownHeading`** 取纯标题），编辑态入口为 **`buildChaptersFromMarkdownEditorText`**；应用章节规则后重载当前文件时以视口末行恢复阅读位置（与 `useAppReaderUiPrefs` 切换排版一致）。
...
- **`useAppFullscreenReaderLayout.ts`**：全屏时正文区域宽度样式；layout 上点击左右空白聚焦编辑器（点击模式则 `beginClickModePointerGesture`：松开翻页、拖动滚动）；两侧空白区 `wheel` 转交 `ReaderMain.delegateEditorWheelFromBrowserEvent`（见下文「全屏正文宽度与两侧空白滚轮」）；事件来自侧栏子树时不劫持（含 Shadow DOM 向上判定）。
...
- **`useAppTimedScroll.ts`**：顶栏 **定时滚动**：`setInterval` 驱动 **`scrollByPageStep` / `scrollByLineStep`**（「一屏」含完整行重叠）；与朗读、编辑、加载、到底互斥；设置变更时重启计时器（见 **「定时滚动」**）。
...
- **`appUi.ts`**：UI 常量：存储 key、侧栏宽度、字号/行间距/字间距/段间距上下限与步进、`default*` 出厂默认等（无本地设置或与 `persistKey` 字段缺失时；见下文「阅读器字号与行间距」「界面与阅读偏好默认值」）；re-export `readerPalette` 的 `applyReaderSurfaceToDocument` 等。
...
- **`readerMainMonaco.css`**（由 `ReaderMain` 引入）：小地图左侧阴影、滚动条轨道只读透明 / 编辑模式实心 `--reader-bg`（滑块半透明、仍可点可拖）、概览尺层级（光标标记不被轨道遮挡）；全屏时小地图/滚动条/概览尺 `position: fixed` 贴视口右缘（见 **`appShell.css`**）。全屏 / 极简只读竖条与概览尺在 `.fullscreen--cursorHidden` 时淡出（编辑模式除外）。阅读器 **`roundedSelection: false`**，避免透明底色下 `.cslr` 抠图层盖住背景图。
- **`readerImageViewZones.ts`**：块级 `![…](…)` 删行并插 ViewZone；返回删行前行号供流管道同步映射；与 `colortxt://` 本地资源协议衔接。
- **`readerKeyScroll.ts`**：只读空格翻页（**`onSpacePageDown`** → **`scrollByPageStep(1)`**）；方向键 / PageUp / PageDown 由快捷键系统调同一套 **`scrollByPageStep` / `scrollByLineStep`**，不在此文件内实现翻页步长。
...
- **`shortcutService.ts`**：窗口级快捷键监听：按持久化绑定匹配并派发动作；**`EDIT_MODE_MONACO_DEFERRED_ACTIONS`**（编辑态 Monaco 内让出滚屏/查找等）；蒙版打开时 **`READER_SCROLL_SHORTCUT_ACTIONS`**（行滚/翻页/上下章）不拦截，其它动作仍生效；侧栏焦点经 **`shouldDeferShortcutForReaderSidebar`** 只让出列表导航与裸 `F2` / 输入框，其余动作仍派发；**`hasDismissibleOverlay`** 时整表不派发（顶栏/底栏/侧栏弹出菜单，仅 Esc 关菜单）；朗读中空格暂停/播放、左右换行，**`VOICE_READ_SCROLL_BLOCKED_ACTIONS`** 仍吞掉行滚/页滚/章节/查找；快捷键面板录制时 **`setWindowShortcutRecordingHandler`** 接管捕获监听，避免已绑定快捷键抢走按键。详见 [基础功能.md](./基础功能.md) → **「快捷键」**、**「Monaco 查找栏」**。
...
- **`cacheStore.ts`**：localStorage：`PersistedSettingsData` / 会话快照等解析与校验（含 **`fileCategory` / `fileSort` / `fileCategoryCatalog`**）。读盘时跑 **`migrateLegacyReaderPaletteOverrides`**，迁完写回并删 **`readerPaletteOverridesLight` / `Dark`**。
...
- **`webDavFindBookSync.ts`**：找书书架（一书一文件增量）、书源、设置。**`UI_READER_KEYS`** 含配色 **`readerPaletteSelectedIdLight` / `Dark`**、**`readerPaletteUserPresets`**、token 开关、**`readerBackground`**，以及旧 **`readerPaletteOverridesLight` / `Dark`**（仅给尚未迁 selectedId 的远端，读盘后迁移消化）。设置同步同样走自定义背景图增量（与主窗共用远端目录）。
...
| `VirtualList.vue`                                    | 虚拟列表（长列表性能）。侧栏列表可开 **`arrowNav`**：↑↓ / PageUp / PageDown / Home / End 移动光标并 `scrollToIndex`，焦点留在滚动容器，避免虚拟行回收后失焦。Enter / Space 点击当前行按钮。 |
