...
│       ├── stealthSettingsMain.ts / StealthSettingsApp.vue # 摸鱼设置（常规 + 快捷键录制，样式对齐 ShortcutPanel）
│       ├── eyedropper.css
...
- **`stealthReaderSettings.ts`**：摸鱼偏好 **`colorTxt.stealth.settings`** 读写与默认值（字体默认 **`system-ui`**、行间距默认 **1.4**、背景不透明度 **0.5** 等）；**`STEALTH_TERMINAL_FONT_MARKER`**（`ui-monospace`）展开为可渲染等宽栈；快捷键字段与 **`@shared/stealthNavShortcuts`** 对齐。产品行为见 [基础功能.md](./基础功能.md) → **「摸鱼模式」**。
...
- **右键菜单**：原生菜单「设置」→ **`openStealthSettingsWindow`**；「退出摸鱼模式」。
