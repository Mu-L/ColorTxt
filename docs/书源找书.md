...
两个「更多」：阅读器**顶栏**（书籍信息 / 编辑书源 / 清除缓存）vs **阅读工具栏**（窄屏字体/格式组、极简视图、查找、设置、配色）。找书窗口顶栏「更多」含快捷键 / 设置 / 配色等。顶栏「刷新」「登录」已外置为图标按钮，勿与主应用「清除缓存」
（清 localStorage）混淆。
...
- 找书窗口自带设置 / **配色（仅阅读器页）** / **快捷键** / 主题 / 书源管理 / 免责声明；启用 WebDAV 后顶栏主题色左侧有 **WebDAV** 菜单（分项同步书架 / 书源 / 设置，见 [基础功能.md](./基础功能.md) → **「WebDAV 同步」**）。切换主题色快捷键与主界面相同（默认 **F2**）。顶栏「更多」与阅读工具栏「更多」均可打开配色。阅读工具栏「更多」窄屏时收纳的字体/格式组在**最上方**（与主界面 `MoreMenu` `#toolbar` 一致），其下为「极简视图」「查找」（走 Monaco 查找栏）。顶栏「更多」另含检查更新 / GitHub / 关于（与主界面同组件）；「快捷键」复用 `ShortcutPanel`（`panelContext=findBook`：无打开文件 / 选择目录 / 章节规则 / 书签 / 侧栏搜索 / 侧栏文件 / 章节 / AI 阅读助手；有书源管理），写入共用 `colorTxt.ui.settings`。
...
- **极简视图** `isMinimalistView` 同样写入找书设置；入口在阅读器「更多」菜单「查找」上方（分隔线隔开）；已在极简时该项为「退出极简视图」；默认快捷键与主界面相同（`F10`）；不跨窗口同步。极简 / 全屏下 **`toggleSidebar`**（默认 `Ctrl+B`）只唤出浮动侧栏，不改 `showSidebar` 持久化。`Esc` 与主界面相同：先关蒙版 / 菜单 / 查找栏 / 浮动面板；全屏须连按两次才退出，不退出极简。浮动顶栏阴影加在 `findBookReaderHeaderWrap` 上，**不再**给 wrap 额外下边框（工具栏自身已有 `border-bottom`）。退出极简后章节列表再居中当前章（与退出全屏相同）。
