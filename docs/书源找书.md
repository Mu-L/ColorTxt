...
└── renderer/src/
    ...
    └── bookSource/
        ├── components/
        │   ├── FindBookPanel.vue           # 壳：三 Tab + 叠层（含朗读设置）
        ...
        │   ├── useFindBookChapterSession.ts    # 章节加载/渲染/编辑进出（含 scrollTo 顶/底）
...
| `localStorage: colortxt:voiceReadSpeak` | 朗读过滤规则与自动暂停（主界面 / 找书共用；不随朗读方案） |
...
| 找书设置相关 localStorage | `colortxt.findBook.settings`：缓存/下载目录、网络代理、找书阅读器侧栏等；**阅读 / 编辑 / 语音朗读 / 词典 / 边界滚动切章**与主界面共用 `colorTxt.ui.settings`；**朗读过滤 / 自动暂停**共用 `colortxt:voiceReadSpeak` |
...
└── VoiceReadSpeakSettingsPanel  ← 叠层（朗读过滤 / 自动暂停）
```
...
两个「更多」：阅读器**顶栏**（书籍信息 / 编辑书源 / 清除缓存）vs **阅读工具栏**（窄屏字体/格式组、查找、设置、配色）。找书窗口顶栏「更多」含快捷键 / 设置 / 配色等。顶栏「刷新」「登录」已外置为图标按钮，勿与主应用「清除缓存」
（清 localStorage）混淆。
...
- Rhino/Mozilla `let (x = expr) body` 表达式：V8 `Unexpected identifier`；改写为
  `((x = expr), (body))`。 
...
| `bookSource:fetchUrl` / `fetchUrlAbort` / `readFile` | 网络/本地 JSON 导入；`fetchUrl` 可选 **`requestId`**，`fetchUrlAbort` 取消对应拉取 |
...
- 找书窗口自带设置 / **配色（仅阅读器页）** / **快捷键** / 主题 / 书源管理 / 免责声明；启用 WebDAV 后顶栏主题色左侧有 **WebDAV** 菜单（分项同步书架 / 书源 / 设置，见 [基础功能.md](./基础功能.md) → **「WebDAV 同步」**）。切换主题色快捷键与主界面相同（默认 **F2**）。顶栏「更多」与阅读工具栏「更多」均可打开配色。阅读工具栏「更多」窄屏时收纳的字体/格式组在**最上方**（与主界面 `MoreMenu` `#toolbar` 一致），其下为「查找」（走 Monaco 查找栏）。顶栏「更多」另含检查更新 / GitHub / 关于（与主界面同组件）；「快捷键」复用 `ShortcutPanel`（`panelContext=findBook`：无打开文件 / 选择目录 / 章节规则 / 书签 / 侧栏搜索 / 侧栏文件 / 章节 / AI 阅读助手；有书源管理），写入共用 `colorTxt.ui.settings`。
- 找书设置标签：「下载」「阅读」「编辑」「语音朗读」「代理」（网络代理，见 §6.11）。主界面「设置 → 代理」与找书「代理」**共用**同一份配置。「阅读 / 编辑 / 语音朗读」与主界面共用 `colorTxt.ui.settings`（各窗内存独立，打开窗口时读最后保存值）；编辑页隐藏「自动刷新章节列表」与「AI 智能排版」。语音朗读页底 **朗读设置**（过滤 / 自动暂停）与主界面同一 **`colortxt:voiceReadSpeak`**。
...
- **导入**：本地 `.json`、网络 URL、或剪贴板 JSON（Legado 导出格式，数组或单对象）；后两者与本地导入相同，走导入预览再提交。书源 WebDAV 同步见找书顶栏 **WebDAV** 菜单「上传 / 同步书源」（远端 `FindBook/bookSource.json`；同步为按 URL 合并，不删本地多出的源）。
  - **网络导入**：`useBookSource.importFromNetwork` 经 **`appLoading`** 显示「正在获取书源 / 解析 / 准备预览」；拉取失败最多再重试 **5** 次（间隔 1s，文案「获取失败，正在重试 n/5」）；**Esc** 中止当前请求（`bookSource:fetchUrlAbort` + `AbortSignal`，不关掉书源面板）。`bookSource:fetchUrl` 可带 **`requestId`**，主进程按 id 取消；底层仍走 **`fetchViaChromiumNet`**（undici 失败回退 Chromium）。
...
- **启用 / 排序 / 分组 / 登录 / 校验**：默认仅启用文本源参与搜索；**特指某一书源搜索**（`sourceUrls`）时不要求该源已启用；
  校验走 `checkSourceService`（并发数可在「校验设置」中配置，默认 **4** 路、上限 32，与找书搜索默认相同）。点停止后不再等待当前书源跑完超时（默认 180s）；进行中的项不标失败、不写入失效分组，列表上的中间态（如「获取正文」）会清空。
- 列表底栏勾选计数为 **当前筛选结果中已勾选数 / 当前显示数**（被过滤掉的不计入）；批量删除/导出/启停等亦仅作用于当前显示中的勾选项。当前列表有校验失败项时，「反选」左侧出现 **选中失败项**（只勾选当前筛选结果中的失败项）。
...
- 复用主应用阅读能力：字体/行距/主题/语音/全屏/极简/定时滚屏等；**阅读 / 编辑 / 语音朗读**与主界面共用 `colorTxt.ui.settings`（各窗内存独立，打开窗口时加载最后保存值，不做运行时互相同步）。朗读过滤 / 自动暂停见 [语音朗读](./语音朗读.md) → **「朗读设置」**。
- **滚动**（平滑滚动 / 滚动倍率 / 滚动加速倍率）与主窗口共用同一份持久化设置。章内空格 / `PageDown` / `PageUp` 走 **`ReaderMain.scrollByPageStep`**（相邻两页重叠一条完整视觉行，见 [基础功能.md](./基础功能.md) → **「黏性章节条」**）。
- 底栏（`FindBookReaderFooter`）：左侧可挂番茄时钟（与主界面同款，设置在「阅读」页；关闭阅读器时自动停止）；自动暂停已开且正在播放时，「阅读进度」左侧同样显示 **`语音朗读：…`**（章节模式按**文档 / 自动续章**计，不是主窗侧栏目录）。右侧展示 `阅读进度：{章进度%}（{正序章号}/{总章数}，{当前行}/{总行}）` 与 `当前章节字数`（去空白后 `formatCharCount`）；章进度按阅读正序下标相对总章数计算（与主界面按全文行/滚动进度不同）。全屏 / 极简时与章节导航一并收入底栏浮层（贴底感应显隐，同主界面）。
- **边界滚动切章**（**`findBookChapterAdvanceEnabled`**，默认 **开启**，写入共用 **`colorTxt.ui.settings`**）：找书阅读器专用；主界面「设置 → 阅读」**不展示**（`SettingsReadingPanel` 的 **`showFindBookChapterAdvanceOption`**）。入口：找书 **设置 → 阅读 → 边界滚动切章**。
  - **语义**：已在本章底部再向下、或已在顶部再向上时进入邻章。下一章落到章首；上一章落到章末（`loadChapterAtDisplayIndex({ scrollTo: "bottom" })`，无平滑滚动）。工具栏 / `Ctrl+←` `Ctrl+→` 的「上一章 / 下一章」仍从章首进入。**阅读尺开启时**：落到章首的切章把章节标题滚到视口中间并作为聚焦带中间行；落到章末的向上边界切章仍在章末，聚焦带贴文末（锚点取带的自然中间行）。
  - **输入**：滚轮（阅读区 `@wheel.capture`，累计阈值防误触，只认正文区域；**阅读尺开启时**滚轮移锚点，尺无法再移即切章，与翻页相同）；空格 / `PageDown` / `PageUp`（阅读尺开启时按「聚焦行数」移锚点，尺已贴边再切章）/ 方向键（阅读尺开启时锚点移 1 行，贴边后再切章）；**点击模式**下阅读区松开左/右键翻页（按下拖动则滚动；**阅读尺开启时**左右键按聚焦行数移锚点）。空格与翻页快捷键、点击翻页均走 `ReaderMain.scrollByPageStep`（贴边时经 **`interceptReadonlyPageStep`** 切章）；方向键走 `scrollByLineStep`（找书在贴边时 `tryAdvanceChapterOnScroll` 先切章）；阅读尺滚轮贴边同样走 **`interceptReadonlyPageStep`**。不要用 Monaco `onDidScrollChange` 检测超额滚动：到底后 `scrollTop` 不再变化。
  - **不切章**：加载中、编辑模式、语音朗读导航锁定。定时滚动 / 语音朗读续章仍走 `advanceToNextChapterForAutoRead`。
  - 实现：`FindBookReaderPanel.tryAdvanceChapterFromOverscroll`；常量 `constants/findBookChapterAdvance.ts`。
- **侧栏开关** `showSidebar` 写入 `colortxt.findBook.settings`（缺省用默认值）；打开阅读器时若正文章节仅 1 章则**临时**收起侧栏（不改持久化），用户手动切换后仍会持久化。
- **极简视图** `isMinimalistView` 同样写入找书设置；入口在阅读工具栏全屏按钮左侧的 **`MinimalistViewButton`**（全屏浮动顶栏同样显示，此时按钮禁用，`F10` 亦不切换）；已进入时为激活的「退出极简视图」；默认快捷键与主界面相同（`F10`）；不跨窗口同步。极简 / 全屏下 **`toggleSidebar`**（默认 `Ctrl+B`）只唤出浮动侧栏，不改 `showSidebar` 持久化。`Esc` 与主界面相同：先关蒙版 / 菜单 / 查找栏 / 浮动面板；全屏须连按两次才退出，不退出极简。浮动顶栏阴影加在 `findBookReaderHeaderWrap` 上，**不再**给 wrap 额外下边框（工具栏自身已有 `border-bottom`）。退出极简后章节列表再居中当前章（与退出全屏相同）。
- **目录附加信息** `showChapterTag`（`BookChapter.tag` / `ruleToc.updateTime`）默认关闭；详情页「目录」与阅读器侧栏「章节」旁可切换，写入同一找书设置。
- 阅读工具栏：支持「编辑模式」与「保存」（写入当前章 `book_cache` 缓存文件；无「智能排版」）；只读时「编辑模式」右侧与主界面相同的 **可选/点击** 交互切换。内存中始终保留原文；**编辑模式直接编辑原文**（不跑文本替换/转换）。阅读模式右键 / **`Ctrl+E`**：**编辑选中文本**（≤1000 字；按展示管线物理行改写并写回章节缓存，再刷新展示）
  ；阅读态经展示管线再渲染。
...
阅读展示顺序：找书为 `文本替换 → 转换（简繁/全半角）→ 压缩空行 / 首行缩进`；主窗口为 `压缩空行 / 首行缩进（format）→ 文本替换 → 转换`（见 [基础功能.md](./基础功能.md) → **「只读展示管线」**）。
...
- **作用于标题 / 作用于正文**（`scopeTitle` / `scopeContent`）：对齐 Legado 分作用域。  
  - **找书**：标题走 **`applyTitleReplaceWithRules`**（章节列表经 **`refreshConvertedChapterTitles`**；正文首行标题同路径）；正文走 **`applyContentReplaceWithRules`**（`useFindBookChapterSession.renderChapterText`）。  
  - **主窗口**：格式化之后、「转换」之前调用 **`applyReplaceRulesToDisplayText`**：正文规则套全文；**章节标题展示行**（`chapterTitleDisplayLineByPhysical`）恢复为套正文前原文后再只套标题规则。`.txt` 侧栏从展示正文匹配章节；`.md` 物理行定结构；只读侧栏 **`title` 取已去 `#` 的展示行**（含「作用于标题」），编辑态展示行仍带 ATX 时经 **`detectMarkdownHeading`** 取纯标题（`buildChaptersFromMarkdownPhysicalLines`）。  
- 纯函数：`@shared/bookSource/replaceRuleApply.ts`（含 **`filterEnabledReplaceRules`**、**`applyReplaceRulesToDisplayText`** 等）。  
  - 找书：`useFindBookChapterSession`（由 `FindBookReaderPanel` 接线）。  
  - 主窗口：`useTxtStreamPipeline`。  
...
- **编辑模式**：工具栏按钮为「格式化：文本替换」；面板主按钮为「应用」= **保存规则** + 对当前编辑缓冲区套用已启用的**正文**替换规则（与「格式化：压缩空行 / 转换」
  同类，写回 Monaco；不按标题作用域分流）。
...
- 入口：
  - 找书窗口「更多 → 设置 → 代理」
  - 主界面「设置 → 代理」（复用同一 **`FindBookSettingsProxyPanel`**；找书专属「书源规则 header / URL 中的 proxy…」提示仅在找书侧显示）
- 支持 HTTP / SOCKS5 / SOCKS4；可选用户名密码；格式对齐 Legado `scheme://host:port[@user@pass]`。
- 持久化在 `colortxt.findBook.settings` 的 `proxy` 字段；变更经 **`saveFindBookProxySettingsAndSync`** 写入，并 IPC 同步到主进程
  `httpProxy.setDefaultBookSourceProxy`；同窗 / 跨窗可通过 `storage` + **`findBookProxyChangedEvent`** 刷新草稿。
- 主界面启动时 **`syncPersistedFindBookProxyToMain`**，保证未打开找书窗时 Wiktionary / Wikipedia 等联网查词也能走代理。
- 生效范围：书源 `fetchStrResponse`（默认 undici，失败回退 Chromium `session.fetch`）、封面拉取，以及词典联网请求（`fetchViaChromiumNet`）；
...
| 全局文本替换（ReplaceRule） | 已实现：管理 / 导入 / 阅读展示套用（导出 `.txt` 不套用）；**作用于标题/正文**分作用域（主窗 `applyReplaceRulesToDisplayText`，找书标题/正文分路径）；正则「替换为」支持 `@js:`（`result` 绑定）；正则超时打断仍未对齐 |
...
**排错建议**：以 legado-E 源码 + Legado_Max 帮助 md 为准；社区教程（喵公子、破冰）作规则语义与样例补充；用具体 `.json` 书源 + 主进程 `logs` 数组调试。
...
| 面板壳 / 叠层 | `FindBookPanel.vue`、`utils/modalStack.ts`、**`VoiceReadSpeakSettingsPanel.vue`**（朗读过滤 / 自动暂停） |
| 书架 | `findBookBookshelf.ts`、`FindBookshelfPanel.vue`、`bookshelfOpenReader.ts` |
| 详情 / 阅读器 | `BookDetailPanel.vue`、`FindBookReaderPanel.vue`（编排，含边界滚动切章）、`FindBookReaderChapterSidebar.vue`、`FindBookReaderFooter.vue`（番茄 / 朗读倒计时 / 章进度）、`useFindBookChapterSession.ts`、`constants/findBookChapterAdvance.ts` |
...
| 找书设置 | `useFindBookSettings.ts`、`FindBookSettings*.vue`；阅读尺等阅读 UI 键与主窗共用 **`colorTxt.ui.settings`**（见 [基础功能.md](./基础功能.md) → **「阅读尺」**） |
