...
- **边界滚动切章**（**`findBookChapterAdvanceEnabled`**，默认 **开启**，写入共用 **`colorTxt.ui.settings`**）：找书阅读器专用；主界面「设置 → 阅读」**不展示**（`SettingsReadingPanel` 的 **`showFindBookChapterAdvanceOption`**）。入口：找书 **设置 → 阅读 → 边界滚动切章**。
  - **语义**：已在本章底部再向下、或已在顶部再向上时进入邻章。下一章落到章首；上一章落到章末（`loadChapterAtDisplayIndex({ scrollTo: "bottom" })`，无平滑滚动）。工具栏 / `Ctrl+←` `Ctrl+→` 的「上一章 / 下一章」仍从章首进入。
  - **输入**：滚轮（阅读区 `@wheel.capture`，累计阈值防误触，只认正文区域）；空格 / `PageDown` / `PageUp` / 方向键（「到边界后再操作一次」）；**点击模式**下阅读区松开左/右键翻页（按下拖动则滚动）。空格走 `ReaderMain.interceptReadonlySpacePageDown`；翻页快捷键走 `useFindBookReaderShortcuts.tryAdvanceChapterOnScroll`；点击翻页走 `ReaderMain.interceptReadonlyPageStep`。不要用 Monaco `onDidScrollChange` 检测超额滚动：到底后 `scrollTop` 不再变化。
