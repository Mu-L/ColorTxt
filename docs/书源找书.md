...
- 找书窗口自带设置 / **配色（仅阅读器页）** / **快捷键** / 主题 / 书源管理 / 免责声明；启用 WebDAV 后顶栏主题色左侧有 **WebDAV** 菜单（分项同步书架 / 书源 / 设置，见 [基础功能.md](./基础功能.md) → **「WebDAV 同步」**）。切换主题色快捷键与主界面相同（默认 **F2**）。顶栏「更多」与阅读工具栏「更多」均可打开配色。阅读工具栏「更多」窄屏时收纳的字体/格式组在**最上方**（与主界面 `MoreMenu` `#toolbar` 一致），其下为「极简视图」「查找」（走 Monaco 查找栏）。顶栏「更多」另含检查更新 / GitHub / 关于（与主界面同组件）；「快捷键」复用 `ShortcutPanel`（`panelContext=findBook`：无打开文件 / 选择目录 / 章节规则 / 书签 / 侧栏搜索；有书源管理），写入共用 `colorTxt.ui.settings`。
...
- **极简视图** `isMinimalistView` 同样写入找书设置；入口在阅读器「更多」菜单「查找」上方（分隔线隔开）；已在极简时该项为「退出极简视图」；默认快捷键与主界面相同（`F10`）；不跨窗口同步。极简 / 全屏下 **`toggleSidebar`**（默认 `Ctrl+B`）只唤出浮动侧栏，不改 `showSidebar` 持久化。
