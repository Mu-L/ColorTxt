...
- 找书窗口自带设置 / **配色（仅阅读器页）** / **快捷键** / 主题 / 书源管理 / 免责声明；启用 WebDAV 后顶栏主题色左侧有 **WebDAV** 菜单（分项同步书架 / 书源 / 设置，见 [基础功能.md](./基础功能.md) → **「WebDAV 同步」**）。顶栏「更多」与阅读工具栏「更多」均可打开配色。阅读工具栏「更多」最上方为「查找」（与主界面一致，走 Monaco 查找栏）。顶栏「更多」另含检查更新 / GitHub / 关于（与主界面同组件）；「快捷键」复用 `ShortcutPanel`（`panelContext=findBook`：无打开文件 / 选择目录 / 章节规则 / 书签 / 侧栏搜索；有书源管理），写入共用 `colorTxt.ui.settings`。
