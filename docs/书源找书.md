...
- **极简视图** `isMinimalistView` 同样写入找书设置；入口在阅读器「更多」菜单「查找」上方（分隔线隔开）；已在极简时该项为「退出极简视图」；默认快捷键与主界面相同（`F10`）；不跨窗口同步。极简 / 全屏下 **`toggleSidebar`**（默认 `Ctrl+B`）只唤出浮动侧栏，不改 `showSidebar` 持久化。`Esc` 与主界面相同：先关蒙版 / 菜单 / 查找栏 / 浮动面板；全屏须连按两次才退出，不退出极简。
